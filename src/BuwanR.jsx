...
function BuwanR() {
  ...
  function validate() {
    ...
    if (!form.Events.trim()) newErrors.Events = 'Event is required.';
    if (!form.email.trim()) {
  ...
  return (
    <div className="ticket-card px-8 py-10 sm:px-12 sm:py-12 max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        ...
        <div>
          <label className="block eyebrow mb-2 text-ink/60">Event</label>
          <input
            type="text"
            name="Events"
            value={form.Events}
            onChange={handleChange}
            placeholder="e.g. Spoken Word"
            className="w-full bg-transparent border-b-2 border-line px-1 py-2 text-ink placeholder:text-ink/30 focus:outline-none focus:border-maroon transition-colors"
          />
          {errors.Events && <p className="text-maroon text-sm mt-1.5">{errors.Events}</p>}
        </div>
